...
class GoalAchievingEndpoints:
    ...
    def create_desire(self, authentication: Authentication, desire: Desire):
        ...
        return {"message": "Desire created successfully", "desire_id": self.cursor.lastrowid}, 200

    @router.get("/api/desires/{desire_id}")
    def get_desire(self, authentication: Authentication, desire_id: int):
        user_id = authentication.user_id
        if not self.user_endpoints.authenticate(authentication):
            raise HTTPException(detail="User is not authenticated, please log in", status_code=401)
        self.cursor.execute("SELECT * FROM desires WHERE desire_id = %s", (desire_id,))
    ...
    def update_desire(self, authentication: Authentication, desire_id: int, updated_desire: Desire):
        self.get_desire(desire_id)
    ...
    def delete_desire(self, authentication: Authentication, desire_id: int):
        user_id = authentication.user_id
        if not self.user_endpoints.authenticate(authentication):
            raise HTTPException(detail="User is not authenticated, please log in", status_code=401)
        self.cursor.execute("SELECT * FROM desires WHERE desire_id = %s", (desire_id,))
        ...
        self.cursor.execute("DELETE FROM desires WHERE desire_id = %s", (desire_id,))
    ...
    def create_goal(self, authentication: Authentication, goal: Goal):
        user_id = authentication.user_id
        if not self.user_endpoints.authenticate(authentication):
            raise HTTPException(detail="User is not authenticated, please log in", status_code=401)
    ...
    def get_goal(self, authentication: Authentication, goal_id: int):
        user_id = authentication.user_id
        if not self.user_endpoints.authenticate(authentication):
            raise HTTPException(detail="User is not authenticated, please log in", status_code=401)
        self.cursor.execute("SELECT * FROM goals WHERE goal_id = %s", (goal_id,))
        res = self.cursor.fetchone()
        if res is None:
    ...
    def update_goal(self, authentication: Authentication, goal_id: int, updated_goal: Goal):
        goal_dict = self.get_goal(authentication, goal_id)[0]
        self.cursor.execute("UPDATE goals SET desire_id = %s, name = %s, how_much = %s, measuring_units = %s, end_instant = %s WHERE goal_id = %s",
    ...
    def delete_goal(self, authentication: Authentication, goal_id: int):
        if not self.user_endpoints.authenticate(authentication):
            raise HTTPException(detail="User is not authenticated, please log in", status_code=401)
        self.cursor.execute("SELECT * FROM goals WHERE goal_id = %s", (goal_id,))
        res = self.cursor.fetchone()
    ...
    def create_plan(self, authentication: Authentication, plan: Plan):
        user_id = authentication.user_id
        if not self.user_endpoints.authenticate(authentication):
            raise HTTPException(detail="User is not authenticated, please log in", status_code=401)
    ...
    def get_plan(self, authentication: Authentication, plan_id: int):
        user_id = authentication.user_id
        if not self.user_endpoints.authenticate(authentication):
            raise HTTPException(detail="User is not authenticated, please log in", status_code=401)
        self.cursor.execute("SELECT * FROM plans WHERE plan_id = %s", (plan_id,))
        res = self.cursor.fetchone()
        if res is None:
    ...
    def update_plan(self, authentication: Authentication, plan_id: int, updated_plan: Plan):
        old_plan_dict = self.get_plan(authentication, plan_id)[0]
        self.cursor.execute("UPDATE plans SET goal_id = %s, event_id = %s, how_much = %s WHERE plan_id = %s;",
    ...
    def delete_plan(self, authentication: Authentication, plan_id: int):
        if not self.user_endpoints.authenticate(authentication):
            raise HTTPException(detail="User is not authenticated, please log in", status_code=401)
        self.get_plan(authentication, plan_id)
        self.cursor.execute("DELETE FROM plans WHERE plan_id = %s", (plan_id,))
    ...
    def create_action(self, authentication: Authentication, action: Action):
        self.get_plan(authentication, action.planId)  # authenticate
    ...
    def get_action(self, authentication: Authentication, action_id: int):
        user_id = authentication.user_id
        if not self.user_endpoints.authenticate(authentication):
            raise HTTPException(detail="User is not authenticated, please log in", status_code=401)
        self.cursor.execute("SELECT * FROM actions WHERE action_id = %s", (action_id,))
        res = self.cursor.fetchone()
        if res is None:
    ...
    def update_action(self, authentication: Authentication, action_id: int, updated_action: Action):
        self.get_action(authentication, action_id)  # authenticate
        self.cursor.execute("UPDATE actions SET successful = %s, how_much_accomplished = %s, notes = %s WHERE action_id = %s",
    ...
    def delete_action(self, authentication: Authentication, action_id: int):
        self.get_action(authentication, action_id)  # authenticate
        self.cursor.execute("DELETE FROM actions WHERE action_id = %s", (action_id,))
        return "success", 200
