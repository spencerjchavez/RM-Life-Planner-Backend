...
class CalendarToDoEndpoints:
    ...
    def create_todo(self, authentication: Authentication, todo: ToDo):
    ...
    def get_todo(self, authentication: Authentication, todo_id: int):
        if not self.user_endpoints.authenticate(authentication):
            raise HTTPException(detail="User is not authenticated, please log in", status_code=401)
    ...
    @router.get("/api/calendar/todos")
    def get_todos(self, authentication: Authentication, start_day: float, end_day: Optional[float] = None):
        if not self.user_endpoints.authenticate(authentication):
            raise HTTPException(detail="User is not authenticated, please log in", status_code=401)
        ...
        while dt.timestamp() <= end_day:
            ...
            self.recurrence_endpoints.register_month_accessed_by_user(authentication, year, month)
    ...
    def update_calendar_todo(self, authentication: Authentication, todo_id: int, todo: ToDo):
    ...
    def delete_todo(self, authentication: Authentication, todo_id: int):
    ...
    def delete_todos_of_user(self, authentication: Authentication):
        if not self.user_endpoints.authenticate(authentication):
            raise HTTPException(detail="User is not authenticated, please log in", status_code=401)
