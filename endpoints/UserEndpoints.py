...
class UserEndpoints:
    ...
    def register_user(user: User):
        ...
        UserEndpoints.users_cursor.execute(sql, (user.username, hashed_password, user.email, date_joined, user.google_calendar_id, salt))

        user_id = UserEndpoints.users_cursor.lastrowid
        UserEndpoints.users_cursor.execute(
    ...
    def isUsernameInUse(username: str):
        UserEndpoints.users_cursor.execute("SELECT * FROM user_ids WHERE username = %s;", (username,))
        return {"is_in_use": "True" if UserEndpoints.users_cursor.fetchone() is not None else "False"}

    @staticmethod
    @router.post("/api/users/login")
    def login_user(username: str, password: str):
        UserEndpoints.users_cursor.execute(f"SELECT * FROM user_ids WHERE username = %s;", (username,))
        res = UserEndpoints.users_cursor.fetchone()
        if res is None:
            raise HTTPException(detail=username + ": no such user exists", status_code=400)
        user_id = res["user_id"]
        UserEndpoints.users_cursor.execute(f"SELECT * FROM users WHERE user_id = %s;", (user_id.__str__(),))
        res = UserEndpoints.users_cursor.fetchone()
        ...
        if res["hashed_password"] == hashed_password:
            ...
            return {"user_id": user_id, "api_key": UserEndpoints.gen_api_key(user_id)}, 200
    ...
    def get_user(user_id: int, api_key: str):
        if not authenticate(user_id, api_key):
            raise HTTPException(status_code=401, detail="User is not authenticated, please log in")
        UserEndpoints.users_cursor.execute("SELECT * FROM users WHERE user_id = %s;", (user_id.__str__(),))
        user = UserEndpoints.users_cursor.fetchone()
    ...
    def update_user(user_id: int, api_key: str, updated_user: User):
        res = UserEndpoints.get_user(user_id, api_key)
        original_user: User = json.loads(res[0]["user"])
        ...
        UserEndpoints.users_cursor.execute(f"UPDATE users SET (password = %s, email = %s, google_calendar_id = %s)", (updated_user.password, updated_user.email, updated_user.google_calendar_id))
    ...
    def delete_user(user_id: int, api_key: str):
        if not authenticate(user_id, api_key):
            raise HTTPException(status_code=401, detail="User is not authenticated, please log in")
        UserEndpoints.users_cursor.execute(f"DELETE FROM users WHERE user_id = %s;", (user_id.__str__(),))
        UserEndpoints.users_cursor.execute(f"DELETE FROM user_ids WHERE user_id = %s;", (user_id.__str__(),))
        try:
            pass
